...
pub struct CreateUserDto {
    ...
    #[validate(
        ...
        regex(path = "USERNAME_REGEX", message = "Username must not contain special characters or spaces")
    )]
    pub username: String,

    #[validate(
        length(min = 6, max = 255, message = "Password must be between 6 and 255 characters"),
    )]
    pub password: String,

    #[validate(
        regex(path = "EMAIL_REGEX", message = "Email must be a valid email")
    )]
    pub email: String,
}
